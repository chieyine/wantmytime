...
var exportSections = []struct{ name, sql string }{
	...
	{"seller_profile", `SELECT to_jsonb(t) FROM (SELECT handle,mode,publication_state,timezone,identity_url,minimum_notice_minutes,booking_horizon_days,buffer_minutes,paused,created_at,(avatar_mime IS NOT NULL) AS has_photo FROM seller_profiles WHERE id=$2) t`},
	...
	{"announcement_emails", `SELECT COALESCE((SELECT to_jsonb(t) FROM (SELECT mc.subscribed,mc.source,mc.wording,mc.confirmed_at,mc.consented_at,mc.unsubscribed_at,(SELECT COALESCE(jsonb_agg(jsonb_build_object('subscribed',e.subscribed,'source',e.source,'wording',e.wording,'at',e.created_at) ORDER BY e.created_at),'[]') FROM marketing_consent_events e WHERE e.email=mc.email) AS history FROM marketing_contacts mc WHERE mc.email=(SELECT normalized_identifier FROM user_identities WHERE user_id=$1 AND type='email' LIMIT 1)) t),'null'::jsonb)`},
...
func (a *API) deleteAccount(w http.ResponseWriter, r *http.Request) {
	...
	steps := []struct{ sql string }{
		...
		{`DELETE FROM broadcast_deliveries WHERE email=$5`},
		{`DELETE FROM marketing_consent_events WHERE email=$5`},
		{`DELETE FROM marketing_contacts WHERE email=$5 OR user_id=$1`},
