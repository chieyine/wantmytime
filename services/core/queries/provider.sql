...
WHERE provider = 'kora' AND currency = sqlc.arg(currency) AND channel = sqlc.arg(channel) AND approved_at IS NOT NULL AND effective_from <= now()
...
VALUES (gen_random_uuid(), sqlc.arg(quote_id)::uuid, 'kora', sqlc.arg(environment), sqlc.arg(reference), sqlc.arg(expected_minor), sqlc.arg(currency)::text, 'initializing', sqlc.arg(approved_fee_minor)::bigint, sqlc.arg(fee_basis_points)::int, sqlc.arg(channel)::text, sqlc.arg(buyer_fee_minor)::bigint)
