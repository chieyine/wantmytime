...
export const GET: RequestHandler = async ({ params, fetch }) => {
	...
	if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(handle) || !/^[0-9]+$/.test(params.version))
	...
	const pixel = (x: number, y: number, color: [number, number, number, number]) => {
	...
	rect(72, 82, 1056, 2, [23, 24, 23, 255]);
	rect(72, 545, 1056, 2, [23, 24, 23, 255]);
	const draw = (
		text: string,
		x: number,
		y: number,
		...
		for (let ci = 0; ci < normalized.length; ci++) {
			const char = normalized[ci];
			if (char === ' ') continue;
			const glyph = glyphs[char] || glyphs['?'];
			for (let gy = 0; gy < 7; gy++)
				for (let gx = 0; gx < 5; gx++)
					if (glyph[gy][gx] === '1') rect(x + ci * 6 * scale + gx * scale, y + gy * scale, scale, scale, color);
		}
	};
	const dark: [number, number, number, number] = [23, 24, 23, 255],
		muted: [number, number, number, number] = [85, 90, 83, 255],
		red: [number, number, number, number] = [180, 58, 48, 255];
	...
	return new Response(png, {
		headers: {
			...
			ETag: `"${handle}-${params.version}-png"`,
