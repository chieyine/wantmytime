<script lang="ts">
	...
	async function load(cursor = '') {
		...
		try {
			...
			const page = await api<{ bookings: Booking[]; next_cursor: string }>(`/api/v1/me/bookings${q}`);
...
<section class="form-page">
	...
	{#if loading && !bookings.length}<p class="page-intro">
			Loading your bookings…
		</p>{:else if message && !bookings.length}<div class="notice notice-warning">
			{message}
			...
			{#each bookings as booking}<a class="list-card" href={`/booking/${encodeURIComponent(booking.id)}`}
					><div>
						...
						<small>{booking.state} · {booking.payment_state}</small>
					</div>
					<strong>{formatMoney(booking.amount_minor, booking.currency)}</strong></a
				>{/each}
		</div>
		<CursorPager cursor={nextCursor} busy={loading} onNext={() => load(nextCursor)} />{/if}
	<p class="form-note">
		Want a copy of your data, or your account deleted? <a class="text-link" href="/app/settings/data"
			>Sign in to manage your data ↗</a
		>
	</p>
</section>
