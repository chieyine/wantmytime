<script lang="ts">
	...
	async function submit() {
		...
		message = '';
		try {
			const challenge = await api<{ challenge_id: string }>('/api/v1/access/challenges', {
				method: 'POST',
			...
			message = e instanceof Error ? e.message : 'A verification code could not be sent.';
		} finally {
			busy = false;
		}
	}
...
<section class="form-page">
	...
	<h1 class="page-heading">Find your booking.</h1>
	<p class="page-intro">Enter the email you booked with. We’ll send a code, then show every booking made with it.</p>
	<form
		class="setup-form"
		onsubmit={(e) => {
			e.preventDefault();
	...
	>
		<label>Email<input class="field" type="email" bind:value={email} autocomplete="email" required /></label>
