<script lang="ts">
	...
	const active = (offer: Offer) =>
		offer.state === 'agreed' ||
		(['pending', 'countered'].includes(offer.state) && new Date(offer.expires_at).getTime() > Date.now());
	let filtered = $derived(
		offers
			.filter((offer) => (filter === 'respond' ? canRespond(offer) : filter === 'active' ? active(offer) : true))
			.sort((a, b) => {
	...
	function status(offer: Offer) {
		...
		if (['pending', 'countered'].includes(offer.state) && new Date(offer.expires_at).getTime() <= Date.now())
...
<section class="form-page app-page workspace-records">
	...
	<p class="page-intro">
		When you let people name a price, their offers land here. Accept one and they get a link to pick a time and pay.
	</p>
	<div class="record-filter" role="group" aria-label="Filter loaded offers">
		<button
			type="button"
			class:active={filter === 'respond'}
			aria-pressed={filter === 'respond'}
			onclick={() => (filter = 'respond')}>TO RESPOND</button
		><button
			type="button"
		...
		><button
			...
			onclick={() => (filter = 'all')}>ALL LOADED</button
		>
	</div>
	...
	{:else if offers.length === 0}<div class="empty-state">
			<span class="empty-line"></span>
			<h2>NO OFFERS YET.</h2>
			<p>Offers people send you will show up here.</p>
		</div>
	{:else}
		{#if filtered.length}<div class="record-list" aria-live="polite">
				{#each filtered as offer (offer.id)}<a class="record-row" href={`/app/offers/${encodeURIComponent(offer.id)}`}
						><div class="record-row-date"><strong>{offer.duration_minutes}</strong><span>MIN</span></div>
						<div class="record-row-main">
							<span class="record-status" class:urgent={canRespond(offer)}>{status(offer)}</span>
							<h2>{offer.role === 'seller' ? offer.buyer_name : offer.seller}</h2>
							<p>
								{offer.role === 'seller' ? 'Received' : 'Sent'} · {offer.state === 'agreed'
									? 'agreement saved'
						...
						<div class="record-row-end">
							<strong>{formatMoney(Number(offer.amount_minor), offer.currency)}</strong><span
								>{nextAction(offer)} ↗</span
							>
						</div></a
					>{/each}
			</div>
		{:else}<div class="record-filter-empty">
				<h2>{filter === 'respond' ? 'YOU ARE ALL CAUGHT UP.' : 'NO OFFERS IN THIS VIEW.'}</h2>
				<p>Nothing here. Older offers may be further down: try All or load more.</p>
			</div>{/if}
