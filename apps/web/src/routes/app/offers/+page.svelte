...
<section class="form-page app-page workspace-records">
	...
	<p class="page-intro">See who is waiting for an answer. Agreeing to an offer does not create a booking or collect payment.</p>
	...
	{:else if offers.length === 0}<div class="empty-state"><span class="empty-line"></span><h2>NO OFFERS YET.</h2><p>Verified offers sent to your link will appear here.</p></div>
	{:else}
		...
		{:else}<div class="record-filter-empty"><h2>{filter === 'respond' ? 'YOU ARE ALL CAUGHT UP.' : 'NO OFFERS IN THIS VIEW.'}</h2><p>Other offers may be on older pages. Use All loaded or load more records below.</p></div>{/if}
