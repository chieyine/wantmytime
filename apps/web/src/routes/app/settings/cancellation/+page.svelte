<script lang="ts">
	...
	async function save() {
		...
		message = '';
		try {
			const p = await api<Policy>('/api/v1/me/cancellation-policy', {
				method: 'PUT',
				body: JSON.stringify({ policy: chosen })
			});
			saved = p.key;
...
<section class="form-page app-page">
	...
	<p class="page-intro">
		Choose how much a buyer is refunded if they cancel. Buyers see it before they pay. If you cancel, or don’t show up,
		the buyer is always refunded in full and there’s no payout for that booking.
	</p>
	{#if options.length}
		<form
			class="setup-form"
		...
		>
			...
			<p class="form-note">
				Every policy gives a full refund to a buyer who cancels within an hour of booking, if the time is at least a day
				away.
			</p>
