...
<section class="form-page app-page workspace-records">
	...
	{:else}
		...
		{:else}<div class="record-filter-empty"><h2>{filter === 'attention' ? 'NO UPCOMING BOOKINGS ON THIS PAGE.' : 'NO BOOKINGS IN THIS VIEW.'}</h2><p>Other bookings may be on older pages. Use All loaded or load more records below.</p></div>{/if}
