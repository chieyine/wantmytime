<script lang="ts">
	...
	let filtered = $derived(
		bookings
			.filter((booking) =>
				filter === 'attention' ? needsAttention(booking) : filter === 'upcoming' ? upcoming(booking) : true
			)
			.sort((a, b) =>
				filter === 'all'
...
<section class="form-page app-page workspace-records">
	...
	<div class="record-filter" role="group" aria-label="Filter loaded bookings">
		<button
			type="button"
			class:active={filter === 'attention'}
			aria-pressed={filter === 'attention'}
			onclick={() => (filter = 'attention')}>YOUR UPCOMING</button
		><button
			type="button"
			class:active={filter === 'upcoming'}
			aria-pressed={filter === 'upcoming'}
			onclick={() => (filter = 'upcoming')}>ALL UPCOMING</button
		><button
			type="button"
	...
	{:else if bookings.length === 0}<div class="empty-state">
			...
			<a href="/app/share" class="text-link">SHARE YOUR LINK ↗</a>
		</div>
	{:else}
		...
		{:else}<div class="record-filter-empty">
				<h2>{filter === 'attention' ? 'NO UPCOMING BOOKINGS ON THIS PAGE.' : 'NO BOOKINGS IN THIS VIEW.'}</h2>
				<p>Nothing here. Older bookings may be further down: try All or load more.</p>
			</div>{/if}
