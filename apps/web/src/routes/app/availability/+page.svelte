...
<section class="form-page app-page workspace-availability">
	...
	{:else}
		<form onsubmit={(event) => { event.preventDefault(); save(); }}>
			...
			<div class="availability-bottom-grid"><section><p class="workspace-kicker">02 / BOOKING LIMITS</p><h2>Leave room around it.</h2><div class="availability-limits"><label>MINIMUM NOTICE <span>MINUTES</span><input class="field" type="number" min="0" max="10080" bind:value={notice} required /></label><label>BOOKING HORIZON <span>DAYS</span><input class="field" type="number" min="1" max="365" bind:value={horizon} required /></label><label>BUFFER AFTER A MEETING <span>MINUTES</span><input class="field" type="number" min="0" max="240" bind:value={buffer} required /></label></div></section><aside><p class="workspace-kicker">YOUR LOCAL VIEW</p><p>Your weekly hours are in <strong>{timezone}</strong>. People booking from anywhere see the same times converted to their own timezone.</p><div class="availability-day-track" aria-hidden="true"><span>00:00</span><span>12:00</span><span>24:00</span>{#each selectedWindows as window}<i style={rangeStyle(window)}></i>{/each}</div></aside></div>
