				...
				{#each payouts as p (p.id)}
					...
					<article class="list-card">
						<div>
							...
							<p>{s.detail}</p>
							{#if p.recovery_minor > 0}<small>{formatNaira(p.recovery_minor)} kept to cover an earlier refund.</small>{/if}
							<small><a class="text-link" href={`/app/bookings/${encodeURIComponent(p.booking_id)}`}>View booking</a></small>
						</div>
