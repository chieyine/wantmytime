<script lang="ts">
	...
	async function load() {
		try {
			reviews = (await api<{ reviews: Review[] }>('/api/v1/ops/reviews')).reviews;
		} catch (e) {
			message = e instanceof Error ? e.message : 'Reviews could not be loaded.';
		}
	}
	onMount(load);
	...
	async function setHidden(id: string, hide: boolean) {
		...
		message = '';
		try {
			await api(`/api/v1/ops/reviews/${encodeURIComponent(id)}/${hide ? 'hide' : 'restore'}`, {
				method: 'POST',
			...
			message = e instanceof Error ? e.message : 'The review could not be updated.';
		} finally {
			busy = '';
		}
	}
...
<section class="form-page app-page">
	...
	<p class="page-intro">
		Only buyers who had a session can review, once per booking. Hide a review only for abuse, personal data or clear
		falsehood, and record why. Both actions are audited.
	</p>
				...
				<div>
					...
					<div class="setup-form">
						...
						<button
							...
							onclick={() => setHidden(r.id, !r.hidden_at)}
							disabled={busy === r.id || (reasons[r.id] ?? '').trim().length < 8}
							>{r.hidden_at ? 'Restore' : 'Hide'}</button
						>
					</div>
				</div>
