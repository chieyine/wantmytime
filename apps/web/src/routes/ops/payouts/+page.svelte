<script lang="ts">
	...
	const stateLabels: Record<string, string> = { scheduled: 'Scheduled', processing: 'Sending', paid: 'Paid', failed: 'Failed', cancelled: 'Nothing to pay' };

	async function load() {
		try {
			const data = await api<{ payouts: Payout[]; paused: boolean }>(`/api/v1/ops/payouts${onlyAttention ? '?state=attention' : ''}`);
	...
	async function retry(id: string) {
		...
		message = '';
		try {
			await api(`/api/v1/ops/payouts/${encodeURIComponent(id)}/retry`, { method: 'POST', body: JSON.stringify({ reason: reasons[id] ?? '' }) });
...
<section class="form-page app-page">
	...
	<p class="page-intro">Each paid booking’s seller share is held, then transferred from the platform balance after the buyer’s problem window closes. A payout waits (it does not fail) while the balance has not settled or the seller has no bank account. Failed and reversed transfers need a retry once the cause is fixed.</p>
	{#if paused}<p class="notice notice-warning">PAYOUTS_PAUSED is on: no transfers are being sent.</p>{/if}
	<label class="form-note"><input type="checkbox" bind:checked={onlyAttention} onchange={load} /> Only payouts that need attention</label>
	{#if message}<p class="notice notice-info" aria-live="polite">{message}</p>{/if}
	{#if payouts.length === 0}<p class="page-intro">{onlyAttention ? 'Nothing needs attention.' : 'No payouts yet.'}</p>{/if}
				...
				<div>
					...
					{#if p.state === 'failed'}
						<div class="setup-form">
							<label>What was checked<input class="field" bind:value={reasons[p.id]} placeholder="e.g. Seller updated their bank account" /></label>
							<button class="button" onclick={() => retry(p.id)} disabled={busy === p.id || (reasons[p.id] ?? '').trim().length < 8}>Retry payout</button>
						</div>
					{/if}
				</div>
