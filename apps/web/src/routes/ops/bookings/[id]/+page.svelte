<script lang="ts">
	...
	type Booking = { id: string; seller: string; buyer_name: string; guest_email: string; duration_minutes: number; starts_at: string; gross_minor: number; currency: string; state: string; payment_state: string; created_at: string; meeting_deadline: string; meeting_ready_at: string | null; issue_reason: string | null; issue_resolved: boolean; issue_reported_by: string; issue_resolution: string; payout: Payout | null };
...
<article class="list-card"><div><strong>Reported problem{booking.issue_reported_by ? ` (by the ${booking.issue_reported_by})` : ''}</strong><p>{booking.issue_reason || 'No problem reported'}</p><small>{booking.issue_resolved ? `Resolved${booking.issue_resolution ? `: ${booking.issue_resolution}` : ''}` : booking.issue_reason ? 'Open' : '—'}</small>
{#if booking.issue_reason && !booking.issue_resolved}
	<form class="setup-form" onsubmit={(e) => { e.preventDefault(); resolve(); }}>
		...
		{#if booking.payment_state === 'paid'}<label>Refund to the buyer, in naira (optional)<input class="field" inputmode="decimal" bind:value={refundNaira} placeholder="0" /></label>
		<p class="form-note">{refundMinor > 0 ? `${formatNaira(refundMinor)} is refunded; the seller’s share of it comes out of their payout, and the rest is released.` : 'No refund: the full payout is released.'}</p>{/if}
