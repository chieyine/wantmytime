<script lang="ts">
	...
	type Funnels = { days: number; buyer: Step[]; seller: Step[]; median_hours_to_first_paid: number | null; includes_simulated_payments: boolean; notes: string[] };
	...
	async function loadFunnels(d: number) {
		days = d;
		funnelMessage = '';
		try { funnels = await api<Funnels>(`/api/v1/ops/funnels?days=${d}`); }
		catch (error) { funnelMessage = error instanceof Error ? error.message : 'Funnels could not be loaded.'; }
	}
	onMount(async () => {
...
<section class="form-page app-page">
	...
	<div class="funnel-range" role="group" aria-label="Period">
		{#each [7, 30, 90] as d}<button type="button" class:selected={days === d} aria-pressed={days === d} onclick={() => loadFunnels(d)}>Last {d} days</button>{/each}
	</div>
	{#if funnels}
		{#each [{ title: 'Buyers: from page view to session', steps: funnels.buyer, of: 'of views' }, { title: 'Sellers: from claimed link to paid booking', steps: funnels.seller, of: 'of claimed' }] as f}
			...
			<section class="readiness-panel funnel">
				...
				<table class="funnel-table">
					<thead><tr><th scope="col">Step</th><th scope="col"><span class="sr-only">Share of first step</span></th><th scope="col">Count</th><th scope="col">From previous</th></tr></thead>
					<tbody>
						{#each f.steps as step, i}
							<tr title={`${step.label}: ${step.count} ${step.basis}, ${pct(step.count, f.steps[0].count)} ${f.of}`}>
								...
								<td class="funnel-bar-cell" aria-hidden="true"><span class="funnel-bar" style:width={`${w[i]}%`}></span></td>
				...
				<p class="form-note">{pct(f.steps[f.steps.length - 1].count, f.steps[0].count)} {f.of} reached the last step in the last {funnels.days} days.</p>
			</section>
		{/each}
		<p class="form-note">Median time from claiming a link to the first paid booking: {funnels.median_hours_to_first_paid === null ? 'no paid bookings yet' : funnels.median_hours_to_first_paid < 48 ? `${Math.round(funnels.median_hours_to_first_paid)} hours` : `${Math.round(funnels.median_hours_to_first_paid / 24)} days`}.</p>
		{#if funnels.includes_simulated_payments}<p class="notice notice-info">This is not production, so simulated payments count as paid.</p>{/if}
