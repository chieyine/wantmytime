	...
	async function verify() {
		...
		try {
			...
			if (purpose === 'claim') {
				...
				const draft = JSON.parse(raw) as {handle:string; name:string; identity_url:string; mode:'fixed'|'offer'|'both'; base_30_minor:number; durations:number[]; timezone:string; country?:string};
