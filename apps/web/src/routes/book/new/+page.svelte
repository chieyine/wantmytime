<script lang="ts">
	...
	$effect(() => { if (selected && !pickedSent && seller) { pickedSent = true; void recordProductEvent('slot_selected', seller); } });
	onMount(() => { if (seller) void recordProductEvent('booking_started', seller); });
	...
	async function startVerification() {
		...
		try {
			...
			const quote = await api<{ id: string }>('/api/v1/quotes', { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ seller, duration_minutes: duration, name: buyerName.trim(), starts_at: selected }) });
...
<section class="booking-page">
	...
	{:else}
		<header class="booking-heading"><p>Book a call</p><h1>Pick a time with<br /><em>{person.name}.</em></h1></header>
		{#if !person.local_simulator && !person.provider_checkout_enabled}<div class="booking-error">Bookings open soon. Check back in a little while.</div>
		{:else if person.paused || !person.ready}<div class="booking-error">{person.name} isn’t taking bookings right now.</div>
		{:else}
			<div class="booking-grid">
				<div class="booking-form">
					<section class="booking-step"><div class="booking-step-heading"><span>01</span><div><h2>Date and time</h2><p>{#if !differentZone}Times are shown in {zoneCity(viewerZone)} time.{:else if showSellerZone}Times are shown in {person.name}’s time ({zoneCity(person.timezone)}).{:else}Times are shown in your time ({zoneCity(viewerZone)}). {person.name} is in {zoneCity(person.timezone)}.{/if}</p>{#if differentZone}<button type="button" class="link-button" onclick={() => { showSellerZone = !showSellerZone; loadSlots(); }}>{showSellerZone ? 'Show my time' : `Show ${person.name}’s time`}</button>{/if}</div></div>
						<label class="booking-date-label" for="booking-date">Date</label><input id="booking-date" class="booking-date" type="date" bind:value={day} onchange={loadSlots} />
						{#if slotsLoading}<p class="booking-help">Finding available times…</p>
						{:else if slots.length === 0}<p class="booking-empty">{message || 'Nothing free on this day. Try another.'}</p>
						{:else}<div class="booking-times" role="group" aria-label="Available times">{#each slots as slot}<button type="button" class:selected={selected === slot.starts_at} aria-pressed={selected === slot.starts_at} onclick={() => selected = slot.starts_at}>{timeOnly(slot.starts_at, differentZone && showSellerZone ? person.timezone : viewerZone)}</button>{/each}</div>{/if}
					</section>
					<section class="booking-step"><div class="booking-step-heading"><span>02</span><div><h2>You</h2><p>So {person.name} knows who’s coming. Your booking confirmation goes to this email.</p></div></div>
						<div class="booking-fields"><label for="booking-name">Your name</label><input id="booking-name" bind:value={buyerName} maxlength="80" autocomplete="name" placeholder="Your name" /><label for="booking-email">Email</label><input id="booking-email" type="email" bind:value={email} autocomplete="email" placeholder="you@example.com" /></div>
					</section>
