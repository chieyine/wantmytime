	...
	{:else}
		...
		<div class="appointment-slip">
			...
			{#if receipt.viewer_role === 'buyer'}
				{#if (receipt.fee_minor ?? 0) > 0}<div>
						<small>PAYMENT FEE</small><strong>{money(receipt.fee_minor)}</strong>
					</div>{/if}
			...
			{/if}
			{#if (receipt.refunded_minor ?? 0) > 0}<div>
					<small>REFUNDED</small><strong>{money(receipt.refunded_minor)}</strong>
				</div>{/if}
			<div><small>DATE</small><strong>{new Date(receipt.paid_at).toLocaleString()}</strong></div>
			<div>
				<small>BOOKING</small><strong
					>{new Date(receipt.starts_at).toLocaleString()} · {receipt.duration_minutes} minutes</strong
				>
			</div>
			{#if receipt.payment_method}<div>
					<small>PAID WITH</small><strong>{methodLabels[receipt.payment_method] ?? receipt.payment_method}</strong>
				</div>{/if}
			...
			<section class="readiness-panel">
				...
				<a
					...
					onclick={async (event) => {
						...
						await recordProductEvent('seller_cta_clicked', receipt!.seller_handle, { booking_id: receipt!.id });
						await goto('/claim');
					}}>Create your link ↗</a
				>
			</section>
