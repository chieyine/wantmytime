<script lang="ts">
		...
		try {
			...
			if (person.mode === 'fixed') {
				window.location.replace(
					`/book/new?seller=${encodeURIComponent(seller)}&duration=${page.url.searchParams.get('duration') ?? 30}`
				);
				return;
			}
	...
	async function submit() {
		...
		message = '';
		try {
			const challenge = await api<{ challenge_id: string }>('/api/v1/auth/challenges', {
				method: 'POST',
				body: JSON.stringify({ email, purpose: 'guest_offer' })
			});
...
<section class="form-page">
	...
	{:else if person.mode === 'fixed' || person.paused || !person.ready}<div class="notice notice-info">
			{person.name} isn’t taking offers right now. Check back soon.
		</div>
	{:else}
		<p class="page-intro">
			Name your price for time with {person.name}. They can accept, counter once or decline, and you’ll get an email
			either way. You only pay if you both agree, after you pick a time.
		</p>
		<form
			class="setup-form"
			onsubmit={(e) => {
			...
			<label
				...
				<div class="money-input">
					<span>{currencySymbol(currency).trim()}</span><input
						...
						inputmode="decimal"
						required
						aria-label={`Offer in ${currency}`}
					/>
				</div>
				<span class="form-note">In {currency}.</span></label
			>
			<label>Your name<input class="field" bind:value={buyerName} maxlength="80" autocomplete="name" required /></label>
			<label
				>Your email<input class="field" type="email" bind:value={email} autocomplete="email" required /><span
					class="form-note">We send a code to confirm it, then send your offer. Replies come to this address.</span
				></label
			>
