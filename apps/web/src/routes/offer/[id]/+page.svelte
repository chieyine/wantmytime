<script lang="ts">
	...
	async function load() {
		try {
			...
			message =
				e instanceof Error ? e.message : 'This offer is unavailable. Open it with the email you used to make it.';
		} finally {
			loading = false;
		}
	}
		...
		try {
			slots = (
				await api<{ slots: Slot[] }>(
					`/api/v1/people/${encodeURIComponent(offer.seller)}/slots?date=${encodeURIComponent(date)}&duration=${offer.duration_minutes}&tz=${encodeURIComponent(zone)}`
				)
			).slots;
		} catch (e) {
			slots = [];
	...
	async function respond(action: 'accept' | 'counter' | 'decline' | 'withdraw') {
		...
		try {
			...
			await load();
			message =
				action === 'accept' && offer?.role === 'buyer'
					? 'Agreed. Now pick a time and pay to confirm the booking.'
	...
	async function checkout() {
		...
		try {
			...
			const quote = await api<{ id: string }>(`/api/v1/offers/${encodeURIComponent(offer.id)}/checkout`, {
				method: 'POST',
...
<section class="form-page">
	...
	{:else if !offer}<div class="notice notice-warning">
			{message}
			<p><a href={`/access?next=${encodeURIComponent(`/offer/${params.id}`)}`}>Open it with your email ↗</a></p>
		</div>
	{:else}
		<div class="appointment-slip">
			...
			<div><small>LENGTH</small><strong>{offer.duration_minutes} minutes</strong></div>
			<div><small>AMOUNT</small><strong>{money(offer.amount_minor)}</strong></div>
			<div><small>STATUS</small><strong>{offer.state.replaceAll('_', ' ')}</strong></div>
		</div>
		{#if offer.state === 'pending' && offer.role === 'seller'}
			<div class="setup-form">
				<button class="button" onclick={() => respond('accept')} disabled={busy}>Accept offer</button><label
					>Counter once
					<div class="money-input">
						<span>{currencySymbol(currency).trim()}</span><input bind:value={counter} inputmode="decimal" />
					</div></label
				><button class="button button-secondary" onclick={() => respond('counter')} disabled={busy}
					>Send counteroffer</button
				><button class="text-link" onclick={() => respond('decline')} disabled={busy}>Decline</button>
			</div>
		{:else if offer.role === 'buyer' && (offer.state === 'pending' || offer.state === 'countered')}
			{#if offer.state === 'countered'}
				<p class="page-intro">
					{offer.seller_name || offer.seller} came back with {money(offer.amount_minor)}. Accept it to pick a time and
					pay, or decline. Respond by {dateLabel(offer.expires_at)}.
				</p>
				<div class="setup-form">
					<button class="button" onclick={() => respond('accept')} disabled={busy}
						>Accept {money(offer.amount_minor)}</button
					><button class="button button-secondary" onclick={() => respond('decline')} disabled={busy}>Decline</button>
				</div>
			{:else}
				<p class="page-intro">
					Waiting for {offer.seller_name || offer.seller} to answer. We’ll email you. Nothing has been charged.
				</p>
			{/if}
		...
		{:else if offer.state === 'agreed' && offer.role === 'buyer'}
			<p class="page-intro">
				Pick a time{offer.checkout_expires_at ? ` by ${dateLabel(offer.checkout_expires_at)}` : ''} and pay {money(
					offer.amount_minor
				)} to confirm it. Times are in your timezone ({zoneCity(zone)}).
			</p>
			<div class="setup-form">
				...
				<button
					class="button"
					onclick={checkout}
					disabled={!selected || busy || (!offer.local_simulator && !offer.provider_checkout_enabled)}
					>{busy ? 'Holding your time…' : 'Continue to payment'}</button
				>
				{#if !offer.local_simulator && !offer.provider_checkout_enabled}<p class="form-note">
						Payments for this seller aren’t switched on yet. Your agreed price is saved.
					</p>{/if}
