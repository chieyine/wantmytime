...
<section class="person-page">
	...
	<div class="person-grid">
		...
		<div class="person-action"><div class="person-action-head"><span>BOOKING / 01</span><span>{person.timezone || 'Africa/Lagos'}</span></div><h2>{person.mode === 'offer' ? 'MAKE AN OFFER.' : 'FIND A TIME.'}</h2>
			...
			{#if person.paused}<p class="person-unavailable">{person.name} is not taking requests right now.</p>
			{:else if person.mode === 'fixed' && !person.ready}<p class="person-unavailable">Bookings are not open yet. Please check back later.</p>
			{:else}<fieldset class="person-duration"><legend>HOW LONG DO YOU NEED?</legend><div class="person-duration-options">{#each person.durations as d}<button type="button" class:active={duration === d} aria-pressed={duration === d} onclick={() => duration = d}><strong>{d}</strong><span>MINUTES</span></button>{/each}</div></fieldset>
				{#if person.mode === 'fixed'}<div class="person-price"><span>YOUR TOTAL</span><strong>{formatNaira(amount)}</strong></div>{#if preview}<button class="person-cta" disabled>CHOOSE A TIME</button>{:else}<a class="person-cta" href={`/book/new?seller=${encodeURIComponent(person.handle)}&duration=${duration}`}>CHOOSE A TIME <span aria-hidden="true">↗</span></a>{/if}<p class="person-fineprint">Review the time and price before paying. Your booking is confirmed after payment verification.{#if person.cancellation_policy}{' '}{person.cancellation_policy.name} cancellation: {person.cancellation_policy.summary}{/if}</p>
				{:else}<p class="person-offer-copy">Suggest an amount for {duration} minutes. {person.name} can accept, counter, or decline. No payment is taken now.</p>{#if preview}<button class="person-cta" disabled>MAKE AN OFFER</button>{:else}<a class="person-cta" href={`/offer/new?seller=${encodeURIComponent(person.handle)}&duration=${duration}`}>MAKE AN OFFER <span aria-hidden="true">↗</span></a>{/if}{/if}
			{/if}
		</div>
	</div>
	{#if reviews.length}
