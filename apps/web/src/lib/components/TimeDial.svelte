...
<div class:compact class="time-dial" aria-label={`Live clock. ${duration} minute conversation selected.`} role="img">
	<svg viewBox="0 0 600 600" aria-hidden="true" focusable="false">
		...
		<circle class="dial-selection" cx="300" cy="300" r="187" stroke-dasharray={`${circumference * duration / 60} ${circumference}`} style={`--reveal-length: ${circumference * duration / 60}px`} transform="rotate(-90 300 300)" />
		{#each ticks as tick}
			{@const start = endpoint(tick * 6, tick % 5 === 0 ? 246 : 261)}
			{@const end = endpoint(tick * 6, 276)}
			<line class:major={tick % 5 === 0} class="dial-tick" style={`--tick-index: ${tick}`} x1={start.x} y1={start.y} x2={end.x} y2={end.y} />
		{/each}
	...
	<div class="time-dial-caption"><span>YOUR LOCAL TIME</span><span>{String(now.getHours()).padStart(2, '0')}:{String(now.getMinutes()).padStart(2, '0')}:{String(now.getSeconds()).padStart(2, '0')}</span></div>
</div>
