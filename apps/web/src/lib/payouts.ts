...
export function payoutStatus(p: Payout): { label: string; detail: string; tone: string } {
	switch (p.state) {
		case 'paid':
			return {
				label: 'Paid',
				detail: `Sent ${p.paid_at ? when(p.paid_at) : ''} to ${p.bank_name} ••${p.account_last4}`,
				tone: ''
			};
		case 'processing':
		...
		case 'failed':
			return {
				...
				detail:
					'The transfer didn’t go through. Check your payout details: we send it again automatically as soon as they’re updated.',
				tone: 'alert-critical'
			};
		case 'cancelled':
		...
		default:
			...
			if (new Date(p.release_at) > new Date())
				return { label: 'Scheduled', detail: `Due ${when(p.release_at)}`, tone: '' };
