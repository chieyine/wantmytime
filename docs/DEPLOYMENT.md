...
- With Sendly, announcements go through its SMTP relay (`SENDLY_SMTP_USERNAME`, `SENDLY_SMTP_PASSWORD`) so these headers point at WantMyTime's own unsubscribe, which affects news only. Sendly's built-in unsubscribe is not used, because it would also stop sign-in codes and booking emails to that person.
